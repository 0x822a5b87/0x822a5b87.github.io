...
`mysql` 的 `InnoDB` 存储引擎，它的整体思路大概是这样的：

1. 总的来说，它通过 Redo Log 来保证数据一致性，同时通过 Undo Log 来实现 `MVCC`。
2. `InnoDB` 在数据写入时，不会每个 Insert 语句都去更新磁盘，而是在内存中维护了一个独特的 `LRU`（也就是我们的 `Buffer Pool`），`LRU` 中的每一个元素就对应于一个Page。当我们更新数据时，它只会更新我们内存中的数据，等待合适的实际的实际将数据刷新到磁盘。整个过程中，它通过 Redo Log 保证了在那些没有被持久化的数据即使断电也不会丢失。
3. 同时，它会使用一个链表结构，将我们 Undo Log 中的各个版本的日志串起来，在查询时向用户返回根据事务隔离等级的数据。
4. 在查询数据时，它先找到对应的所有的Page（例如直接查询索引），如果Page在 `Buffer Pool` 中，那么就不需要进行IO了。

而在 `pg` 中，它的查询逻辑则完全不同：**最重要的问题在于：`InnoDB` 使用的是 B+树，所有行的数据存储在索引的叶子节点上（当然，只针对于聚簇索引，对于二级索引需要回表），查询到了索引便得到了数据。** 
