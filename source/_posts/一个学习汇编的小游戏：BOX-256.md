...
在我们的实现中，我们使用 $[29, 38]$ 这个区间来表示当前计算节点的前面 `0x10h` 个节点，并递归的计算最终的结果。
...
接下来，我们开始正式的绘制`Sierpinski triangle`，其实这里的逻辑相对来说也比较简单：

1. 绘制一个三角形；
2. 在三角形中找到一个新的顶点，并以这个顶点作为新的起点绘制一个三角形；
3. 此时，由于这个三角形的绘制，我们又分割出来三个三角形，我们再递归的去进行绘制；
4. 满足递归退出条件（三角形的边长为1）后，结束当前递归。
